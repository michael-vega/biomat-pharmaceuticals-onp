---
title: "Biomat Pharmaceuticals ONP 16S PreProcessing Markdown"
output: html_notebook
---

This R markdown notebook describes the pre-processing of partial 16S rRNA gene amplicon sequences generated for the manuscript titled, "Pharmaceutical biotransformation is influenced by photosynthesis and microbial nitrogen cycling in a benthic wetland biomat". Demultiplexed forward and reverse reads can be found on NCBI under BioProject PRJNA818364. Necessary sample names are listed within the file, "Biomat_Pharmaceuticals_ONP_Amplicon_Metadata.csv", which is also used to create the phyloseq object, as well as in Table S3 of the manuscript. 
...
#Creatinf subdirectory and amending names to denote filtering
...
Merging forward and reverse reads, then visualizing them in a mergers table.
```{r}
...
Quantifying the fraction of our sequences that were chimeric.
```{r}
...
Importing phylogenetic tree (.qza) into R and converting to file type compatible with phyloseq.
```{r}
...
write.csv(sample_sums(ps_prok_final),"Total_16S_sample_sums.csv")
```
